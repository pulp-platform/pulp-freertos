...
/* GPIO[31:0] direction configuration bitfield: - bit[i]=1'b0: Input mode for
 * GPIO[i] - bit[i]=1'b1: Output mode for GPIO[i] (access: R/W) */
#define GPIO_PADDIR_DIR_BIT   0
#define GPIO_PADDIR_DIR_WIDTH 32
#define GPIO_PADDIR_DIR_MASK  0xffffffff

/* GPIO[31:0] clock enable configuration bitfield: - bit[i]=1'b0: disable clock
 * for GPIO[i] - bit[i]=1'b1: enable clock for GPIO[i] GPIOs are gathered by
 * groups of 4. The clock gating of one group is done only if all 4 GPIOs are
 * disabled.  Clock must be enabled for a GPIO if it's direction is configured
 * in input mode. (access: R/W) */
#define GPIO_GPIOEN_GPIOEN_BIT	 0
#define GPIO_GPIOEN_GPIOEN_WIDTH 32
#define GPIO_GPIOEN_GPIOEN_MASK	 0xffffffff

/* GPIO[31:0] input data read bitfield. DATA_IN[i] corresponds to input data of
 * GPIO[i]. (access: R) */
#define GPIO_PADIN_DATA_IN_BIT	 0
#define GPIO_PADIN_DATA_IN_WIDTH 32
#define GPIO_PADIN_DATA_IN_MASK	 0xffffffff

/* GPIO[31:0] output data read bitfield. DATA_OUT[i] corresponds to output data
 * set on GPIO[i]. (access: R/W) */
#define GPIO_PADOUT_DATA_OUT_BIT   0
#define GPIO_PADOUT_DATA_OUT_WIDTH 32
#define GPIO_PADOUT_DATA_OUT_MASK  0xffffffff

/* GPIO[31:0] set bitfield: - bit[i]=1'b0: No change for GPIO[i] - bit[i]=1'b1:
 * Sets GPIO[i] to 1 (access: W) */
...
/* GPIO[31:0] clear bitfield: - bit[i]=1'b0: No change for GPIO[i] -
 * bit[i]=1'b1: Clears GPIO[i] (access: W) */
...
/* GPIO[31:0] interrupt enable configuration bitfield: - bit[i]=1'b0: disable
 * interrupt for GPIO[i] - bit[i]=1'b1: enable interrupt for GPIO[i] (access:
 * R/W) */
#define GPIO_INTEN_INTEN_BIT   0
#define GPIO_INTEN_INTEN_WIDTH 32
#define GPIO_INTEN_INTEN_MASK  0xffffffff

/* GPIO[15:0] interrupt type configuration bitfield: - bit[2*i+1:2*i]=2'b00:
 * interrupt on falling edge for GPIO[i] - bit[2*i+1:2*i]=2'b01: interrupt on
 * rising edge for GPIO[i] - bit[2*i+1:2*i]=2'b10: interrupt on rising and
 * falling edge for GPIO[i] - bit[2*i+1:2*i]=2'b11: RFU (access: R/W) */
#define GPIO_INTTYPE0_INTTYPE0_BIT   0
#define GPIO_INTTYPE0_INTTYPE0_WIDTH 32
#define GPIO_INTTYPE0_INTTYPE0_MASK  0xffffffff

/* GPIO[31:16] interrupt type configuration bitfield: - bit[2*i+1:2*i]=2'b00:
 * interrupt on falling edge for GPIO[16+i] - bit[2*i+1:2*i]=2'b01: interrupt on
 * rising edge for GPIO[16+i] - bit[2*i+1:2*i]=2'b10: interrupt on rising and
 * falling edge for GPIO[16+i] - bit[2*i+1:2*i]=2'b11: RFU (access: R/W) */
#define GPIO_INTTYPE1_INTTYPE1_BIT   0
#define GPIO_INTTYPE1_INTTYPE1_WIDTH 32
#define GPIO_INTTYPE1_INTTYPE1_MASK  0xffffffff

/* GPIO[31:0] Interrupt status flags bitfield. INTSTATUS[i]=1 when interrupt
 * received on GPIO[i]. INTSTATUS is cleared when it is red. GPIO interrupt line
 * is also cleared when INTSTATUS register is red. (access: R) */
...
/* GPIO[63:32] input data read bitfield. DATA_IN[i] corresponds to input data of
 * GPIO[i]. (access: R) */
...
/* GPIO[63:32] output data read bitfield. DATA_OUT[i] corresponds to output data
 * set on GPIO[i]. (access: R/W) */
...
/* GPIO[63:32] set bitfield: - bit[i]=1'b0: No change for GPIO[i] - bit[i]=1'b1:
 * Sets GPIO[i] to 1 (access: W) */
...
/* GPIO[63:32] clear bitfield: - bit[i]=1'b0: No change for GPIO[i] -
 * bit[i]=1'b1: Clears GPIO[i] (access: W) */
...
/* GPIO[63:32] interrupt enable configuration bitfield: - bit[i]=1'b0: disable
 * interrupt for GPIO[i] - bit[i]=1'b1: enable interrupt for GPIO[i] (access:
 * R/W) */
...
/* GPIO[47:32] interrupt type configuration bitfield: - bit[2*i+1:2*i]=2'b00:
 * interrupt on falling edge for GPIO[i] - bit[2*i+1:2*i]=2'b01: interrupt on
 * rising edge for GPIO[i] - bit[2*i+1:2*i]=2'b10: interrupt on rising and
 * falling edge for GPIO[i] - bit[2*i+1:2*i]=2'b11: RFU (access: R/W) */
...
/* GPIO[63:48] interrupt type configuration bitfield: - bit[2*i+1:2*i]=2'b00:
 * interrupt on falling edge for GPIO[16+i] - bit[2*i+1:2*i]=2'b01: interrupt on
 * rising edge for GPIO[16+i] - bit[2*i+1:2*i]=2'b10: interrupt on rising and
 * falling edge for GPIO[16+i] - bit[2*i+1:2*i]=2'b11: RFU (access: R/W) */
...
/* GPIO[63:32] Interrupt status flags bitfield. INTSTATUS[i]=1 when interrupt
 * received on GPIO[i]. INTSTATUS is cleared when it is red. GPIO interrupt line
 * is also cleared when INTSTATUS register is red. (access: R) */
