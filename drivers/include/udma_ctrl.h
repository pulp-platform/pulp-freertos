...
{
	hal_or32(&UDMA_GC->CG, 1ul << udma_device_id);
}

static inline void udma_ctrl_cg_enable(uint32_t udma_device_id)
{
	hal_and32(&UDMA_GC->CG, ~(1ul << udma_device_id));
}
...
{
	/* clear current event */
	hal_and32(&UDMA_GC->EVTIN, ~((uint32_t)UDMA_GC_EVTIN_MASK(0xFF)
				     << UDMA_GC_EVTIN_SHIFT_ID(udma_evt_nb)));
	/* set new event */
	hal_or32(&UDMA_GC->EVTIN, ((uint32_t)UDMA_GC_EVTIN_MASK(event_id)
				   << UDMA_GC_EVTIN_SHIFT_ID(udma_evt_nb)));

	/* tell soc eu to forward us the event */
	hal_soc_eu_set_pr_mask((int)event_id);
}
						   ...
						   uint32_t event_id)
{
	...
	hal_soc_eu_clear_pr_mask((int)event_id);
}
