 ...
 */
int cl_delegate_device_read(struct pi_device *device, uint32_t size, void *loc_address,
 ...
 * \param device_address address in device if significant, NULL otherwise
 */
int cl_delegate_device_write(struct pi_device *device, uint32_t size, void *loc_address,
			     uint32_t device_address);
 ...
 */
int cl_delegate_device_read_async(struct pi_device *device, uint32_t size, void *loc_address,
				  uint32_t device_address, void *req);
 ...
 * \param req opaque sync object to wait on for request completion
 */
int cl_delegate_device_write_async(struct pi_device *device, uint32_t size, void *loc_address,
				   uint32_t device_address, void *req);
