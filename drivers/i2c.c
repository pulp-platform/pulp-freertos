...
int pi_i2c_write(struct pi_device *device, uint8_t *tx_data, int length, pi_i2c_xfer_flags_e flags)
{
	...
	pi_task_wait_on_no_mutex(&task_block);
#endif
	return PI_OK; /* TODO: thats just buggy */
}
...
static void __pi_i2c_hw_fifo_enqueue(struct i2c_itf_data_s *driver_data, struct pi_task *task,
				     udma_channel_e channel);
		...
		if (driver_data->pending->pending_repeat) {
			...
			if (driver_data->i2c_stop_send) {
				if (channel == RX_CHANNEL) {
					driver_data->hw_buffer[TX_CHANNEL / TX_CHANNEL] =
						driver_data->hw_buffer[RX_CHANNEL];
