	      ...
	      unsigned int num_bytes)
{
	...
	// Execute a barrier to synchronize the team of cores
...
static void pe_entry(void *arg)
{
	...
	// Each cluster core programs the idma to perform a data transfer slice
	...
	for (int i = 1; i < MAX_BUFFER_SIZE; i *= 2) {
		...
		printf("Starting test for %d transfers from %p to %p\n", i,
		       EXT_DATA_ADDR + (coreid * MAX_BUFFER_SIZE),
		       TCDM_DATA_ADDR + (coreid * MAX_BUFFER_SIZE));
		hal_eu_mutex_unlock(0);

		error += test_idma(TCDM_DATA_ADDR + (coreid * MAX_BUFFER_SIZE),
				   EXT_DATA_ADDR + (coreid * MAX_BUFFER_SIZE),
				   i);
	}
